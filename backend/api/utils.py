...
def get_reactions_post_context(queryset, user):
    ...
    # group by theo 2 field trên và đếm , annotate tính toán ở db nên nhanh hơn, trả ra field total (tính băng related name của user reaction là reactions)
...
def get_reactions_comment_context(queryset, user):
    if hasattr(queryset, 'values_list'):
        comment_ids = list(queryset.values_list('id', flat=True))  # Chú ý: có thể là 'comment_id' nếu bạn đổi tên PK
    else:
        comment_ids = [obj.comment_id if hasattr(obj, 'comment_id') else obj.pk for obj in queryset]
